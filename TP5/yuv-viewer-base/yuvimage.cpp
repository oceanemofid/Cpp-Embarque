...
QRgb convert_yuv_to_rgb(uint8_t y, uint8_t u, uint8_t v) {
  auto yi = static_cast<uint>(y);
  auto ui = static_cast<uint>(u);
  auto vi = static_cast<uint>(v);

  int pyi = 1192 * yi;

  int r = pyi  + 1921 * vi - 274643;
  int g = pyi  - 218 * ui - 571 * vi + 84843;
  int b = pyi  + 2163 * ui - 295953;

  Util::Clamp<int> clamp_to_rgb(0, 255);

  return qRgb( clamp_to_rgb((r+512) >> 10), clamp_to_rgb((g+512) >> 10), clamp_to_rgb((b+512) >> 10));
}
